...
from ..utils import update_many, insert_many
...
class BaseAdapter(object):
    ...
    def update(self):
        ...
        existing = ExchangeRate.objects.values('source__code', 'target__code', 'id')
        ...
        for source in currencies:
            for target in currencies:
                ...
                if (source.code, target.code) in existing:
                    ...
                    logger.debug('exchange rate updated {}/{}={}'.format(source, target, rate))
                else:
                    inserts.append(exchange_rate)
                    logger.debug('exchange rate created {}/{}={}'.format(source, target, rate))

            logger.info('exchange rates updated for {}'.format(source.code))
