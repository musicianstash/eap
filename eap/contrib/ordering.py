...
ORDERING_SETTINGS = getattr(settings, "ORDERING_SETTINGS", {})

DEFAULT_ORDER = ORDERING_SETTINGS.get("DEFAULT_ORDER", 'DESC')
DEFAULT_FIELD = ORDERING_SETTINGS.get("DEFAULT_ORDER", None)


class Ordering(object):
    def __init__(self, object_list, fields, default_field=None,
                 default_order=None, request=None):
        self.object_list = object_list
        self.fields = fields
        self.default_field = default_field or DEFAULT_FIELD
        self.default_order = default_order or DEFAULT_ORDER
        self.base_queryset = self.request.GET.copy()
        self.request = request

    @property
    def orders(self):
        default_field = self.default_field or self.fields[0]

        order_obj_list = []
        for field in self.fields:
            queryset = self.base_queryset.copy()
            is_active = field in queryset

            if field == default_field and is_active:
                del queryset[field]

        return order_obj_list

    @property
    def get_current_selected_order(self):
        pass
...
#
# QuerySetPaginator = Ordering  # For backwards-compatibility.
#
#
